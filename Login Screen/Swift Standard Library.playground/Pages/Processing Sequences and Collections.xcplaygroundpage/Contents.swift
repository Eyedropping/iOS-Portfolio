...
*/
let sum = totalPrices.reduce(1) { currentPrice, priceToAdd in
...
*/
let sortedIngredients = sampleIngredients.sorted { $0.quantity > $1.quantity }
...
if let i = ingredientNames.firstIndex(of: "Rice") {
    ingredientNames[i]
}
 ...
 */
let purchased: Int? = sampleIngredients.firstIndex(where: { $0.purchased })
